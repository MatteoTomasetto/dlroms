...
def train_nested(encoder, decoder, phi, snapshots, param, ntrain, epochs, optim = torch.optim.LBFGS, lr = 1, lossf = None, lossw = 1.0, error = None, verbose = True, until = None, conv = lambda x: "%.2e" % x, best = False, cleanup = True):
    
    autoencoder = encoder + decoder
    optimizer = optim(list(autoencoder.parameters()) + list(phi.parameters()), lr = lr)
    
    ntest = len(snapshots)-ntrain
    snapshots_train, snapshots_test = snapshots[:ntrain], snapshots[-ntest:]
    param_train, param_test = param[:ntrain], param[-ntest:]
    ...
    for e in range(epochs):
        ...
        def closure():
            optimizer.zero_grad()
            loss = lossf(snapshots_train, autoencoder(snapshots_train)) + lossw * lossf(encoder(snapshots_train), phi(param_train))
            loss.backward()
            return loss
        optimizer.step(closure)

        with torch.no_grad():
            if(autoencoder.l2().isnan().item() or phi.l2().isnan().item()):
                break
            errAE.append([error(snapshots_train, autoencoder(snapshots_train)),
                          error(snapshots_test, autoencoder(snapshots_test))])
            errPHI.append([error(encoder(snapshots_train), phi(param_train)),
                          error(encoder(snapshots_test), phi(param_test))])
            if(verbose):
                if(cleanup):
                        clear_output(wait = True)
                print("\t\tTrain\tTest")
                print("Epoch "+ str(e+1) + ":\t" + conv(errAE[-1][0]) + "\t" + conv(errAE[-1][1]) + ".")
                print("Epoch "+ str(e+1) + ":\t" + conv(errPHI[-1][0]) + "\t" + conv(errPHI[-1][1]) + ".")
            if(until!=None):
                if(errAE[-1][0] < until and errPHI[-1][0] < until):
                        break
            if(best and e > 0):
                if(errAE[-1][1] < bestvAE and errPHI[-1][1] < bestvPHI):
                        bestvAE = errAE[-1][1] + 0.0
                        bestvPHI = errPHI[-1][1] + 0.0
                        autoencoder.save("tempAE%d" % tempcode)
                        phi.save("tempPHI%d" % tempcode)

    if(best):
        try:
            autoencoder.load("tempAE%d" % tempcode)
            for file in autoencoder.files("tempAE%d" % tempcode):
                os.remove(file)
            phi.load("tempPHI%d" % tempcode)
            for file in phi.files("tempPHI%d" % tempcode):
                os.remove(file)
        except:
            None
    clock.stop()
