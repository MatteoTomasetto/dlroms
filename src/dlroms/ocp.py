...
def snapshots(n, sampler, core = GPU, verbose = False, folder = None, filenames = ['MU', 'Y', 'U']):
    ...
    mu, y, u = core.tensor(mu), core.tensor(y), core.tensor(u)
    if(folder is None):
        return mu, y, u
    else:
        torch.save(mu, folder + filenames[0].replace(".pt",""))
        torch.save(y, folder + filenames[1].replace(".pt",""))
        torch.save(u, folder + filenames[2].replace(".pt",""))